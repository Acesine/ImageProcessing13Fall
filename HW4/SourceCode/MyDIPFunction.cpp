...
void Dilation4(const unsigned char *src, unsigned char *tar, int SizeI, int SizeJ,char type)
{
    switch (type)
    {
    case 'L':
...
void Erode4(const unsigned char *src, unsigned char *tar, int SizeI, int SizeJ,char type)
{
    switch(type)
    {
    case 'L':
    ...
    case 'D':
        for(int i=0; i<SizeI; i++)
        {
            for(int j=0; j<SizeJ; j++)
            {
                ...
                if(src[i*SizeJ+j] == 0)
                {
                    if(i-1>=0 && src[(i-1)*SizeJ+j] == 0)
                    {
                        if(i+1<SizeI && src[(i+1)*SizeJ+j] == 0)
                        {
                            if(j-1>=0 && src[i*SizeJ+j-1] == 0)
                            {
                                if(j+1<SizeJ && src[i*SizeJ+j+1] == 0)
                                {
                                    tar[i*SizeJ+j] = 0;
                                }
                            }
...
int ConnectedComponentLabeling(const unsigned char* src, int *mapping, int SizeI, int SizeJ,int LabelNum_tmp,int *labels)
{
    // object is dark
    // Label equivalence tabel
    int *LET = new int[LabelNum_tmp];
    for(int i=0;i<LabelNum_tmp;i++)
        LET[i] = i;
    // initial label: object:1, background:0
    for(int i=0;i<SizeI;i++)
    {
        for(int j=0;j<SizeJ;j++)
        {
    ...
    int neighbori[] = {-1,-1,-1,0};
    int neighborj[] = {-1,0,1,-1};
    ...
    for(int i=0;i<SizeI;i++)
    {
        for(int j=0;j<SizeJ;j++)
        {
            if(mapping[i*SizeJ+j]!=0)
            {
                int Labelmin = currentLabel;
                int flag = 0;
                for(int k=0;k<4;k++)
                {
                    neighborLabels[k] = 0;
                    if(i+neighbori[k]>=0&&j+neighborj[k]>=0&&j+neighborj[k]<SizeJ)
                    {
                        neighborLabels[k] = mapping[(i+neighbori[k])*SizeJ+j+neighborj[k]];
                        if(neighborLabels[k]!=0)
                        {
                            flag = 1;
                            if(neighborLabels[k]<Labelmin)
                                Labelmin = neighborLabels[k];
                        }
                    }
                }
                if(flag==0)
                {
                    currentLabel ++;
                    mapping[i*SizeJ+j] = currentLabel;
                }
                else
                {
                    mapping[i*SizeJ+j] = Labelmin;
                    for(int k=0;k<4;k++)
                    {
                        if(neighborLabels[k]!=0)
                        {
                            if(LET[neighborLabels[k]]>Labelmin)
                            {
                                LET[neighborLabels[k]]=Labelmin;
                            }
                        }
                    }
                }
    ...
    for(int i=0;i<SizeI;i++)
    {
        for(int j=0;j<SizeJ;j++)
        {
            if(mapping[i*SizeJ+j]!=0)
            {
                mapping[i*SizeJ+j] = LET[mapping[i*SizeJ+j]];
            }
        }
    }
    ...
    for(int i=0;i<SizeI;i++)
    {
        for(int j=0;j<SizeJ;j++)
        {
            if(mapping[i*SizeJ+j]!=0)
            {
                ...
                if(flag == 0)
                {
                    labels[counter] = mapping[i*SizeJ+j];
                    counter++;
                }
            }
        }
...
// Mean-shift
// G(x) = -k'(x), k(.) is the kernel function, ignore constants because they will cancel each other later
float G(int i1, int j1, int i2, int j2, int r1, int g1, int b1, int r2, int g2, int b2, float hs, float hr)
{
    float result;
    result = exp(-0.5*((i1-i2)*(i1-i2)+(j1-j2)*(j1-j2))/(hs*hs)) * exp(-0.5*((r1-r2)*(r1-r2)+(g1-g2)*(g1-g2)+(b1-b2)*(b1-b2))/(hr*hr));
    return result;
}
void MeanShiftFiltering(unsigned char* src, unsigned char* tar, int SizeI, int SizeJ, float hs, float hr, int W)
{
    struct feature{
        float i;
        float j;
    ...
    for(int i=0;i<SizeI;i++)
    {
        ...
        for(int j=0;j<SizeJ;j++)
        {
            struct feature x1, x2;
            x1.i = i;
            x1.j = j;
            x1.r = src[3*(i*SizeJ+j)];
            x1.g = src[3*(i*SizeJ+j)+1];
            x1.b = src[3*(i*SizeJ+j)+2];
            int counter = 0;
            struct feature *x_next;
            struct feature *x_curr;
            while(1)
            {
                if(counter%2==0)
                {
                    ...
                    if(m>=0 && m<SizeI)
                    {
                        for(int n=j-W;n<=j+W;n++)
                        {
                            if(n>=0 && n<SizeJ)
                            {
                                ...
                                t.j = n;
                                t.r = src[3*(m*SizeJ+n)];
                                t.g = src[3*(m*SizeJ+n)+1];
                                t.b = src[3*(m*SizeJ+n)+2];
                                float g = G(x_curr->i,x_curr->j,t.i,t.j,x_curr->r,x_curr->g,x_curr->b,t.r,t.g,t.b,hs,hr);
                                D += g;
                                x_next->i += 1.0 * t.i * g;
                                x_next->j += 1.0 * t.j * g;
                                x_next->r += 1.0 * t.r * g;
                ...
                float Th = (x_next->i-x_curr->i)*(x_next->i-x_curr->i)+(x_next->j-x_curr->j)*(x_next->j-x_curr->j)+
                           (x_next->r-x_curr->r)*(x_next->r-x_curr->r)+(x_next->g-x_curr->g)*(x_next->g-x_curr->g)+
                           (x_next->b-x_curr->b)*(x_next->b-x_curr->b);

                if(Th < 0.001)
                    break;
                counter++;
            }
            tar[3*(i*SizeJ+j)] = x_next->r;
            tar[3*(i*SizeJ+j)+1] = x_next->g;
            tar[3*(i*SizeJ+j)+2] = x_next->b;

        }
    }
    printf("Mean shift filtering done!\n");
}

void MeanShiftSegmentation(unsigned char* src, unsigned char* tar, int SizeI, int SizeJ, float hs, float hr, int W, int M)
{
    struct feature{
        float i;
        float j;
    ...
    struct feature *centers = new struct feature[SizeI*SizeJ];
    int centers_ptr = 0;
    int *centers_num = new int[SizeI*SizeJ];
    int *index = new int[SizeI*SizeJ];
    ...
    for(int i=0;i<SizeI;i++)
    {
        ...
        for(int j=0;j<SizeJ;j++)
        {
            ...
            int flag = 0;
            index[i*SizeJ+j] = -1;
            for(int k=centers_ptr-1;k>=0;k--)
            {
                float ds = sqrt((tmp.i-centers[k].i)*(tmp.i-centers[k].i)+(tmp.j-centers[k].j)*(tmp.j-centers[k].j));
                float dr = sqrt((tmp.r-centers[k].r)*(tmp.r-centers[k].r)+(tmp.g-centers[k].g)*(tmp.g-centers[k].g)+(tmp.b-centers[k].b)*(tmp.b-centers[k].b));
                if(ds<hs && dr<hr)
                {
                    flag = 1;
                    index[i*SizeJ+j] = k;
            ...
            if(flag == 0)
            {
                centers[centers_ptr].i = tmp.i;
                centers[centers_ptr].j = tmp.j;
                centers[centers_ptr].r = tmp.r;
                centers[centers_ptr].g = tmp.g;
                centers[centers_ptr].b = tmp.b;
                index[i*SizeJ+j] = centers_ptr;
                centers_num[centers_ptr] = 0;
                centers_ptr++;
            }
            centers_num[index[i*SizeJ+j]] ++;
        }
    }
    int *remapping = new int[centers_ptr];
    for(int i=0;i<centers_ptr;i++)
    {
        remapping[i] = i;
    }
    ...
    for(int i=0;i<SizeI;i++)
    {
        for(int j=0;j<SizeJ;j++)
        {
            tar[3*(i*SizeJ+j)] = centers[remapping[index[i*SizeJ+j]]].r;
            tar[3*(i*SizeJ+j)+1] = centers[remapping[index[i*SizeJ+j]]].g;
            tar[3*(i*SizeJ+j)+2] = centers[remapping[index[i*SizeJ+j]]].b;
        }
    }
    delete centers;
    delete centers_num;
    delete remapping;
    delete index;
    printf("Mean shift segmentation done!\n");
}

void Gray2BinaryAdaptive(const unsigned char *src, unsigned char *tar,int SizeI, int SizeJ, int S,int C)
{
    for(int i=0; i<SizeI; i++)
    {
        for(int j=0; j<SizeJ; j++)
...
//                            count++;
//                            sum += src[m*SizeJ+n];
                            if(Max<src[m*SizeJ+n])
                                Max = src[m*SizeJ+n];
                            if(Min>src[m*SizeJ+n])
                                Min = src[m*SizeJ+n];
                        }
                    }
                }
            }
            float Thr = 1.0*(Max+Min)/2;
            Thr = Thr-C;
            if(src[i*SizeJ+j]<Thr)
                tar[i*SizeJ+j] = 0;
            else
                tar[i*SizeJ+j] = 255;
        }
