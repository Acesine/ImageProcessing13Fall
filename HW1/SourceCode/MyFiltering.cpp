...
{
    int ExpanedI = SizeI + 2*Margin;
    int ExpanedJ = SizeJ + 2*Margin;
    ...
    for(int color=0; color<BytesPerPixel; color++)
    {
        ...
        for(int i=0; i<SizeI; i++)
        {
            ...
            {
                ImageDataOut[((i+Margin)*ExpanedJ+j+Margin)*BytesPerPixel+color] = ImageData[BytesPerPixel*(i*SizeJ+j)+color];
            }
        }
        // 4 corners
        for(int i=0; i<Margin; i++)
        {
            ...
            {
                ImageDataOut[BytesPerPixel*(i*ExpanedJ+j)+color] = ImageData[color];
                ImageDataOut[BytesPerPixel*(i*ExpanedJ+Margin+SizeJ+j)+color] = ImageData[BytesPerPixel*(SizeJ-1)+color];
                ImageDataOut[BytesPerPixel*((Margin+SizeI+i)*ExpanedJ+j)+color] = ImageData[BytesPerPixel*((SizeI-1)*SizeJ)+color];
                ImageDataOut[BytesPerPixel*((Margin+SizeI+i)*ExpanedJ+Margin+SizeJ+j)+color] = ImageData[BytesPerPixel*(SizeI*SizeJ-1)+color];
            }
        }
        // Top and bottom
        for(int i=0; i<Margin; i++)
        {
            for(int j=Margin; j<ExpanedJ-Margin; j++)
            {
                ImageDataOut[BytesPerPixel*(i*ExpanedJ+j)+color] = ImageData[BytesPerPixel*(j-Margin)+color];
                ImageDataOut[BytesPerPixel*((Margin+SizeI+i)*ExpanedJ+j)+color] = ImageData[BytesPerPixel*((SizeI-1)*SizeJ+j-Margin)+color];
            }
        }
        // Left and right
        for(int i=Margin; i<ExpanedI-Margin; i++)
        {
            for(int j=0; j<Margin; j++)
            {
                ImageDataOut[BytesPerPixel*(i*ExpanedJ+j)+color] = ImageData[BytesPerPixel*(i-Margin)*SizeJ+color];
                ImageDataOut[BytesPerPixel*(i*ExpanedJ+Margin+SizeJ+j)+color] = ImageData[BytesPerPixel*((i-Margin)*SizeJ+SizeJ-1)+color];
            }
        }
    }
}
...
void Filter(const double *FilterData, int FilterSize, const unsigned char* ImageData, unsigned char* ImageDataOut, int SizeI, int SizeJ, int BytesPerPixel)
{
    ...
    int ExpanedI = SizeI + 2*Margin;
    int ExpanedJ = SizeJ + 2*Margin;
    unsigned char ExpandedImage[ExpanedI][ExpanedJ][BytesPerPixel];
    // Construct Expanded Image
    ImageExtendRepeat(Margin, ImageData, &ExpandedImage[0][0][0], SizeI, SizeJ, BytesPerPixel);
...
void MedianFilter(char FilterShape, int FilterSize, const unsigned char* ImageData, unsigned char* ImageDataOut, int SizeI, int SizeJ, int BytesPerPixel)
{
    ...
    WriteImageToFile("ExpandedImage.raw",&ExpandedImage[0][0][0],ExpanedI,ExpanedJ,BytesPerPixel);
...
void BilateralFilter(int FilterSize,double sigma_d, double sigma_r, const unsigned char* ImageData, unsigned char* ImageDataOut, int SizeI, int SizeJ)
{
    ...
    int ExpanedI = SizeI + 2*Margin;
    int ExpanedJ = SizeJ + 2*Margin;
    unsigned char ExpandedGrayImage[ExpanedI][ExpanedJ];

    double DomainFilter[FilterSize][FilterSize];
    double RangeFilter[256];

    // Construct Expanded Image
    ImageExtendRepeat(Margin,ImageData,&ExpandedGrayImage[0][0],SizeI,SizeJ,1);
    ////
